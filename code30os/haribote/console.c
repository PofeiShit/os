...
{
	struct FILEINFO *finfo = (struct FILEINFO *) (ADR_DISKIMG + 0x00200);
	int i, j;
	char s[30];
	for (i = 0; i < 224; i++) {
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
	...
	/* ファイルを探す */
	finfo = file_search(name, (struct FILEINFO *) (ADR_DISKIMG + 0x00200), 224);
	if (finfo == 0 && name[i - 1] != '.') {
	...
	if (finfo != 0) {
		...
		if (appsiz >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) {
			...
			task->ds_base = (int) q;
			set_segmdesc(task->ldt + 0, segsiz - 1, (int) p, AR_CODE32_ER + 0x60);
	...
	if (edx == 1) {
		...
		if (i < 8) {
			finfo = file_search((char *) ebx + ds_base,
					(struct FILEINFO *) (ADR_DISKIMG + 0x00200), 224);
			if (finfo != 0) {
				reg[7] = (int) fh;
				fh->size = finfo->size;
				fh->pos = 0;
